...
class ResNet(nn.Module):
    def __init__(
        ...
        self.last = nn.Conv2d(512, 16, kernel_size=1, stride=1, padding=0, bias=False)
...
class SharedSegmentDecoder(nn.Module):
    def __init__(self, num_features=16):
        super(SharedSegmentDecoder, self).__init__()
    ...
    def forward(self, x):
        batch_size = x.size(0)
        x = x.view(batch_size * self.num_features, 1, 16,16)
        segment_maps = self.decoder(x)
        segment_maps = segment_maps.view(batch_size, self.num_features, 256, 256)
        return segment_maps
