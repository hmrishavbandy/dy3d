from utils.utils_spaghetti.options import Options
from utils.utils_spaghetti import models_utils, transformer
import utils.utils_spaghetti.constants as constants
...
class DecompositionControl(models_utils.Model):
    ...
    def forward_split(self, x: T) -> Tuple[T, TS]:
        ...
        # print("Raw GMM shape", raw_gmm.shape)
        # exit()
        split_ = torch.split(raw_gmm, self.split_shape, dim=3)
        # print(len(split_))
        # print(split_[-2].shape)

        # exit()
        gmms = split_gm(split_)
        zh = self.to_s(x)
        zh = zh.view(b, -1, zh.shape[-1])
        return zh, gmms
    
    def raw_to_gmm(self, raw_gmm):
        split_ = torch.split(raw_gmm, self.split_shape, dim=3)
        # print(len(split_))
        # print(split_[-2].shape)
    ...
    def forward_split_v2(self, x: T) -> Tuple[T, TS]:
        b = x.shape[0]
        raw_gmm = self.to_gmm(x).unsqueeze(1)
        # print("Raw GMM shape", raw_gmm.shape)
        # exit()
    ...
    def forward_mid_v2(self, zs) -> Tuple[T, TS]:
        zh, raw_gmm, gmms = self.forward_split_v2(zs)
        return zh, raw_gmm, gmms
...
class Spaghetti(models_utils.Model):
    ...
    def merge_gmms(self, gmms):
        ...
        mu, p, phi, eigen = [item.view(b, gp * g, *item.shape[3:]) for item in gmms]
        p = p.reshape(*p.shape[:2], -1)
        z_gmm_1 = torch.cat((mu, p, phi.unsqueeze(-1), eigen), dim=2).detach()
        return z_gmm_1
    
    
    def merge_zh_v2(self,zh, z_gmm, mask: Optional[T] = None) -> TNS:
        # zh_, attn = self.mixing_network.forward_with_attention(zh_gmm, mask=mask)
        z_gmm = self.from_gmm(z_gmm)
        zh_ = zh + z_gmm
