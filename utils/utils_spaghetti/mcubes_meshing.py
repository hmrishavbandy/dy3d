...
from utils.utils_spaghetti.train_utils import Logger
...
def mcubes_torch(pytorch_3d_occ_tensor: T, voxel_grid_origin: List[float], voxel_size: float) -> T_Mesh:
    ...
    verts, faces = marching_cubes(pytorch_3d_occ_tensor.unsqueeze(0), 0)
    # print()
    verts = verts[0]
    faces = faces[0]
...
class MarchingCubesMeshing:
    ...
    def occ_meshing(self, decoder, res: int = 256, get_time: bool = False, verbose=False, get_occ = False):
        start = time.time()
        voxel_origin = [-1., -1., -1.]
        voxel_size = 2.0 / (res - 1)
